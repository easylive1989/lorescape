...
void main() {
  group('WikidataPlaceCoordsService', () {
    test(
      'given a WKT point in Wikidata order, when the response is parsed, '
      'then longitude comes first and latitude second',
      () async {
    ...
    test(
      'given negative coordinates, when the response is parsed, '
      'then the sign is kept',
      () async {
        final service = WikidataPlaceCoordsService(
          client: MockClient(
            (_) async => http.Response(
    ...
    test(
      'given the device is offline, when coordinates are resolved, '
      'then an empty map comes back instead of an exception',
      () async {
        final service = WikidataPlaceCoordsService(
