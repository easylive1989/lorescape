...
void main() {
  test('given a ring wholly on the near hemisphere, '
      'when clipping it, '
  ...
  test('given a ring wholly on the far hemisphere, '
      'when clipping it, '
  ...
  test('given a ring straddling the horizon, '
      'when clipping it, '
  ...
  test('given a hole ring straddling the horizon, '
      'when clipping it, '
  ...
  test('given a ring whose visible part is split in two by a notch, '
      'when clipping it, '
    ...
    expect(clipped, hasLength(2), reason: '兩段 run 各自封閉，照環上順序接則只會得到一條');
    expect(
      clipped.every(
        (poly) => poly.every((p) => (p - _center).distance <= _radius + 0.01),
      ),
      isTrue,
      reason: '裁切後不該有任何點跑到球外',
    );
