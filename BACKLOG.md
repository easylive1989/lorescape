...
- [ ] 2026-08-04 回顧：檢視補流量主線是否推動流量/下載/留存指標，再決定是否解除暫緩、回補產品側投入（原公司決策設定的檢核點）
  - ⚠️ **逾期 6 天未做**；2026-08-10 開工時補上初步判讀（正式決策仍待使用者拍板）。
  - **主線有推動，但尚未達標。** 目標是落地頁「穩定兩位數/天」；
    8/03–8/09 週 web 活躍 20（≈2.9/天，最高單日 7，8/09），前週僅 5。
    首次脫離個位數週值，但離「穩定兩位數/天」還有一個量級。
  - 上游明顯有效：IG 週觸及 4,113 → **18,977**（+361%）、粉絲 88 → **232**、
    iOS 週下載 4 → **11**。漏斗上緣正在放大。
  - **建議維持暫緩、不要現在解除。** 理由：(a) 落地頁未達標；(b) 本週 70%
    觸及來自兩支離群值（8/05 廣島、8/08 昌德宮），單週數據還不足以證明是
    可重複的成長而非運氣；(c) 真正卡住的兩件事都在既有主線內——App 送審
    （見「待部署」，已 17 天）與片尾 CTA（F15 T4），都不需要解除暫緩就能做。
  - 下次檢核點建議設 **2026-09-01**（與 8/31 reels 期末檢核、月報同批），
    屆時有四週資料可判斷本週的跳躍是不是可重複的。

## ⚠️ 待部署（程式已在 repo，尚未上生產；更新於 2026-08-24）

> **2026-08-24 順檢結果：四個服務裡三個是最新的，只有 publisher 卡了一支。**
>
> | 服務 | 最後 commit | 最後部署 | 狀態 |
> |---|---|---|---|
> | `frontend/` | 2026-08-20 | Deploy App 08-21 01:08 ✅ | 最新（8/21 後無新 commit）|
> | `backend/` | 2026-08-19 | Deploy Backend 08-20 14:46 ✅ | 最新 |
> | `landing/` | 2026-08-20 前 | Deploy Landing 08-24 00:26 ✅ | 最新（每日故事發布會自動觸發）|
> | `publisher/` | **2026-08-14** `e5ca88c0` | **Deploy Publisher 08-13 12:28** | ⚠️ **未部署，已 10 天** |
>
> - [ ] **publisher `e5ca88c0`「fix(publisher): wander cover 四字標題橫向縮放，
>   不再折行」（2026-08-14）尚未上 VPS。** 最後一次 Deploy Publisher 是 8/13，
>   比這支 commit 早一天。
>   **影響範圍應該有限**——wander 圖組是在本機渲染後由 `send_carousel_for_review`
>   上傳成品，VPS 端不重跑 renderer；但 `publish` 貼到 Discord 的**預設 IG 卡片**
>   是 VPS 這邊算的，四字標題的折行問題在那條路徑上仍未修。
>   ⇒ 下次有其他 publisher 改動要上時一併部署即可，不必為它單獨跑一次
>   （預設卡片在現行流程中不會真的發出去，見 lorescape-manual-daily-story Step 8b）。
>
> ADR 0009（恢復付費牆／每日免費一篇）與 ADR 0010（移除 App 端每日故事與
> 收藏地點）的程式改動，均已隨 8/19–8/20 的 backend commit 與 8/21 的
> Deploy App 上生產，**不在待部署清單內**。⚠️ 但 F13 T5 本週發現
> narration 完成率 0，其中一個嫌疑正是 ADR 0009 的免費額度判定誤觸 402，
> 已列為 T5a 的首要查核項。

### 以下為 2026-07-27 的歷史紀錄（已全數結案，保留供追溯）
...
- [x] **App**（`frontend/`）：**2026-08-11 的 build 已送審上架**（使用者 8/17 告知，
  iTunes lookup 查證：版本 `20260811.0050`，`currentVersionReleaseDate`
  `2026-08-11T21:14:56Z` ＝ 台北 8/12 05:14）。下方整串累積改動——F10 相簿權限鍵、
  F13 T1a/T1b 埋點、F18 T7·T8、F17 T8、移除 Always 定位鍵、F31 T2 移除升級
  banner ／ paywall 導向——**均已隨此版本上生產**，先前三週（8/3、8/10、8/17）
  的「尚未送審」順檢紀錄到此結案。
  - 尚待實機驗證：F18 T8 書脊鬼影字（本地重現不出來）。
  - 已驗證埋點運作：`app_funnel.csv` 8/10–8/16 收到 8 次 `first_open_users`。
    ⚠️ 但同期 `narration_start_users` 為 **0**（前週 6）——**那不是埋點問題，
    是產品漏斗問題**，見 F13 新增的 T4。
- [ ] ~~**App**（`frontend/`，下一輪）：待下次 build 送審才生效的累積改動~~
  （**已於 8/11 build 上架，見上一條；以下列表保留當歷史**）——
  ⚠️ 2026-08-03 順檢：自 07-24 上架後已累積近兩週未送審；其中 narration /
  screen_view 埋點修復不上架，GA4 App 數據就一直缺——愈晚送審缺口愈長，
  建議儘快排一次 build 送審。
  ⚠️ **2026-08-10 順檢：仍未送審，距 07-24 上架已 17 天**（上週的建議沒有
  推動）。本週新增兩個變得更急的理由：(a) 8/3–8/9 是帳號量級跳躍週
  （IG 觸及 +361%、iOS 下載 4→11），進站流量正在放大，而 App 端埋點缺口
  讓這批新用戶的行為完全量不到；(b) F31 T4 的升級 banner 仍留在舊版畫面上，
  付費牆已於 8/05 下線但使用者還看得到它。
  ⚠️ ~~2026-08-17 順檢：仍未送審~~ **（此條當日稍後作廢：使用者告知 8/11 build
  已送審上架，見上方 [x] 那條。原文誤把 `app_funnel` 欄位名讀成
  `first_open`／`narration_started`，實際為 `first_open_users`／
  `narration_start_users`，first_open 其實有資料，埋點正常。）**
...
- [ ] T3: 下週週報驗證 reel 片尾下載 CTA 成效（2026-07-13 上線
  ...
  - 2026-07-20 部分驗證：對「追蹤」有效（聖家堂 Reel 24h 帶進 +5 粉絲、
    ...
    見 F15
- [ ] T4（2026-08-17 新增，來源 weekly-2026-08-17）: **本週 8 個人開了 App，
  0 個人開始聽故事。**

  | 週 | first_open_users | narration_start_users | narration_complete_users |
  |---|---|---|---|
  | 8/10–8/16 | 8 | **0** | **0** |
  | 8/03–8/09 | 18 | 6 | 2 |

  **先排除埋點嫌疑**：前週同一組欄位收得到事件，且 8/11 的 build
  （`20260811.0050`，8/12 上架）之後 first_open 照常進來——所以不是漏報，
  是使用者真的沒按下播放。
  - [x] T4a: ~~實機走首次開啟流程~~ **不需要——使用者 2026-08-17 確認流程本身
    沒問題，就是沒人用。** ⇒ 這不是「卡住」的漏斗，是**動機缺口**：人裝了、
    開了，然後沒有理由播下去。
  - **重新定義問題**：從 IG reel 來的人是被故事本身吸引而點擊，但 App 的核心
    使用情境是「站在景點前」——下載當下他們人在沙發上，沒有可播的對象。
    ⇒ 要動的不是首次開啟畫面，是**給一個當場就能用的理由**。可能的方向
    （尚未評估，勿當結論）：首開就給一則不需定位的故事（例如當天的每日故事）、
    或把 F13 T2 的每日故事推播當成主要回訪機制。
  - [x] T4b: ~~下週看 narration_start 是否仍為 0~~ **（2026-08-24 結案：不是 0
    了，恢復到 6。連兩週 0 的條件未成立，但問題換了形狀——見 T5。）**

- [ ] T5（2026-08-24 新增，來源 weekly-2026-08-24）: **「裝了不聽」變成
  「聽了不完」——本週 6 人開始、6 人放棄、0 人聽完。**

  | 週 | first_open | narration_started | completed | abandoned | cohort | D1 留存 |
  |---|---|---|---|---|---|---|
  | 8/17–8/23 | **32** | **6** | **0** | **6** | 42 | 0 |
  | 8/10–8/16 | 8 | 0 | 0 | 0 | 11 | 0 |
  | 8/03–8/09 | 18 | 6 | 2 | — | — | — |

  T4 的「動機缺口」判讀（人裝了但沒理由播）**本週被推翻**：這回有六個人
  真的按了播放。但完成率 0.0%，`narration.csv` 的 abandoned 欄六筆全中。
  對照 8/03–8/09 那週同樣是 6 started 卻有 2 completed，**中離率從 67%
  惡化到 100%**。

  同期 cohort 放大到 42 人、D1 留存仍 0。前幾週 cohort 只有 1–10 人時，
  留存 0 可以推給樣本太小；42 人隔天回訪 0 人，這個數字開始有意義了，
  且與完成率 0 指向同一件事——**首次體驗沒有留住人**。

  - [ ] T5a: **先分辨是「播放失敗」還是「內容中離」，這決定要修程式還是改內容。**
    - 播放失敗的證據長相：abandoned 集中在極短的播放秒數、narration API 有
      402/5xx、TTS 音檔載入錯誤。查法見 lorescape-debug；402 尤其要查——
      付費牆已於 8/19 恢復（ADR 0009，非訂閱者每日免費一篇），**若免費額度
      判定有誤，使用者會在按下播放後撞到 402**，時間點與本週惡化吻合。
    - 內容中離的證據長相：播放秒數分散、集中在故事中段。
  - [ ] T5b: 依 T5a 結果處置。若是 402 誤判 ⇒ 這是 ADR 0009 的迴歸 bug，最優先修。
...
- **2026-08-10 新證據：天平明顯倒向「自家裝置」。** 把 Android 週活躍拉長看：
  ...
  07-22 ～ 08-01 那段連續高值（峰值 07-26 的 16、07-30 的 14）前後都貼近零，
  8/02 之後就回到 1–4/天。**同期 Play 至今仍未匯出任何 installs**
  （`store_android.csv` 的 installs 欄全空，只有 store_performance 有值）。
  一段有頭有尾、與上架時間鬆散相關、且完全沒有安裝數佐證的流量，比較像
  某人那兩週在測機器，不像真實用戶累積後流失。
  - 8/10 已用 `metrics.report --only ga4 --start/--end` 重抓 08-03～08-07 確認
    **不是抓取失敗**：GA4 對零活躍的日期本來就不回傳列（8/03 整日無列、
    8/04·8/05·8/07 的 Android 欄空白皆為真實零值）。
  - **在 F26（濾除內部流量）生效前，不要把 Android 數字寫進任何週報結論。**
    8/10 週報已據此把該區塊標為「來源存疑，不列入判讀」。
...
- [ ] T4（2026-08-01 新增，來源 monthly-2026-07 P0）: 8 月每週檢核
  ...
  - **2026-08-03 條件成立**：7/20 週 1.0% → 7/27 週 0.75%，連兩週 < 1.5%
    ...
    不另起爐灶。
  - **2026-08-10 第三週讀數 0.97%（184 / 18,977），連三週 < 1.5%，但問題的
    性質變了。** 分子從 31 跳到 184（**+494%**），分母從 4,113 跳到 18,977
    （+361%）——比率原地踏步是因為分母漲更快，不是 CTA 完全無效。修正後
    的判讀：CTA **有效但沒有隨觸及等比放大**。
    - 因此 (a) bio 那條的優先度可以降低（bio 是被動入口，本來就跟觸及等比）；
      **(b) 片尾 CTA 才是槓桿所在**。現行片尾是品牌句 +「下載連結在個人簡介」，
      缺一個「為什麼現在要點」的具體理由——這是下一版要補的東西。
    - ⚠️ 判讀警告：本週 70% 的觸及來自 8/5 廣島與 8/8 昌德宮兩支離群值。
      下週若觸及回落，比率可能自動回升到 1.5% 以上，那**不是 CTA 改好了**，
      只是分母縮回去。改版成效要看 profile_views 絕對值的斜率，不是比率。
  - **2026-08-17 第四週讀數 0.46%（30 / 6,570），連四週 < 1.5%，且上週那個
    樂觀判讀被推翻了。** 觸及 18,977→6,570（−65%），profile_views
    184→30（**−84%**）——**分子掉得比分母更兇**。上週說「比率不動是分母漲
    太快、CTA 其實有效」，若成立，這週分母縮回去時比率該回升；實際卻從
    0.97% 腰斬到 0.46%。
    - 修正後的判讀：**8/10 那批 profile_views 是廣島與昌德宮兩支爆量 reel
      帶來的，不是 CTA 本身的效果。** 扣掉離群值後，CTA 的真實轉換一直
      停在 0.5% 上下沒有動過。
    - ⇒ (b) 片尾 CTA 仍是槓桿，但要當成**從未被驗證有效**來重做，不是
      「微調一個已經有效的東西」。下一版必須補上「為什麼現在要點」的
      具體理由，並且**單獨用一支 reel 做前後對照**（同類型、同 style），
      不要一次全改。
    - 來源：`marketing/audits/weekly-2026-08-17.md`
...
- [ ] T4: （長期）恢復分層紙娃娃——2026-08-10 使用者拍板改單張全身圖
  （ADR 0008，破圖 + 骨骼維護成本），骨骼編輯器與 layout.json 已整組
  移除。若日後要恢復：素材需加重疊裕度、恢復 layout 資料層與動畫
  分組，另立 ADR
...
## F32: Carousel 分發近乎歸零，投入產出嚴重失衡 (epic: E1)

- 狀態: 觀察中（2026-08-10 新增；2026-08-17 決定本期不做對照實驗，carousel 照常發）
- 來源: marketing/audits/weekly-2026-08-10.md（P0）

**2026-08-03 ～ 08-09 的對比**：
...
差距 **32 倍**。關鍵不在中位數本身，而在**七則全部落在 39–84 的窄帶內**
——內容題材、hook 品質、發布時間都不同，觸及卻幾乎一樣，這比較像 IG 根本
沒有分發它們，而不是內容好壞的差異。

成本面：wander carousel 每天吃掉一輪完整的產製（Claude 寫 slides.json →
渲染 → 目視確認 → `send_carousel_for_review` → Discord 審核按鈕），
換到的觸及不到 Reels 的 0.5%。

- [ ] T1: **先做對照實驗，不要直接停發。** 挑三天只發 Reel、完全不發
  carousel，比較當日「帳號觸及」與「該支 Reel 觸及」。這能分辨兩種解釋：
  (a) carousel 本身就沒有分發 ⇒ 停發不影響帳號觸及；
  (b) 同日雙發互相稀釋 ⇒ 停發後當日 Reel 觸及應明顯上升。
  兩者的處置完全相反，沒分辨清楚就停發等於白白丟掉一個素材位。
  - 建議挑 Week 3 的 8/18·8/20·8/22（閉合型那三天），讓謎型組不受影響、
    不污染 8/31 期末的 24h 略過率比較。
- [ ] T2: 依 T1 結果決定處置——
  - 若是 (a)：停發或大幅降頻（例如每週兩則精選），把時間投回 Reel 品質；
    此時 lorescape-manual-daily-story Step 8b 的「固定 wander」決策要一併改，
    並更新 2026-07-06 那條固定風格決策的理由。
  - 若是 (b)：改成隔日發、或與 Reel 錯開至少 6 小時。
- [ ] T3: 無論結果為何，把 carousel 觸及加進週報固定追蹤欄位（目前週報只看
  貼文總觸及，carousel 的長期低迷被 Reels 的大數字蓋掉了，這次是靠分型
  才看出來）。

**2026-08-24 第三週讀數：續探底，且 F32 的核心論點再被強化一次。**
8/17–8/23 七則 carousel 觸及中位 **28**（區間 13–85，合計 242），同期 reel
中位 596。七則的總觸及加起來不到單支中位 reel 的一半。連三週的中位是
57 → 28 → 28，**已經沒有起色可等**。

- [ ] T4（2026-08-24 新增，**需使用者拍板**）: **carousel 是否降級為
  「只服務既有粉絲的補充內容」。** 這是決策題不是資料題——T1 的對照實驗
  在 8/17 已由使用者裁示不做，所以「(a) 沒分發 vs (b) 雙發稀釋」永遠不會有
  答案；在沒有答案的情況下，剩下的判斷依據只有投入產出比。
  建議的降級內容：製作流程簡化（Claude 直接產 slides.json 不再逐張審稿）、
  或降頻到每週兩則精選，把省下的時間投回 reel 的 hook 與片尾 CTA
  （那是判準 3 連五週指向的地方）。若降級，
  lorescape-manual-daily-story Step 8b 的「固定 wander」決策要一併改。

**2026-08-17 第二週讀數：又掉一半，且窄帶更窄。** 8/10–8/16 七則 carousel
觸及中位 **28**（區間 23–188，扣掉 8/10 那則 188 後全部落在 23–29），
同期 reel 中位 674。上週的觀察不但沒有反轉，還更極端——**七則裡有六則
落在 23–29 的六點寬窄帶內**，這幾乎不可能是內容差異造成的。

⚠️ **T1 的實驗窗口就是本週（8/18·8/20·8/22），今天不決定就錯過了。**
錯過這期就得等 8/31 之後另找三天，而下期的 style 交錯排程會讓可用日更少。
- [x] T1a（2026-08-17）: 已決定——**本期不做對照實驗，carousel 照常每日發**
  （使用者 8/17 開工時裁示）。⇒ **T1 順延，本期不執行**；F32 整體降為觀察中，
  週報繼續記錄 carousel 觸及中位數，不再每週重提實驗窗口。
  下次要做實驗，最早是 8/31 期末結算之後另找三天。

## F33: 單日觸及落差 43 倍，需釐清是內容還是發布時段 (epic: E1)

- 狀態: 觀察中（2026-08-10 新增）
- 來源: marketing/audits/weekly-2026-08-10.md（P1）

8/08 昌德宮 reach 8,092（48h 略過率 36.4%，全期最低）→ 8/09 莫高窟
reach 186（24h 略過率 61.3%，全期最差）。**相鄰兩天、同一條產線、同樣
的品質流程，觸及差 43 倍。**

莫高窟那支的內容品質沒有明顯問題（王圓籙單主角弧線、初稿破碎已重寫），
所以更可能是發布時段、演算法冷卻，或前一支爆量後的帳號級節流。

- [ ] T1: 比對 `ig_posts.csv` 各支的實際發布時間（`posted_date` 只有日期，
  需另從 IG permalink 或 `social_posts` 的排程時間取時分），看低觸及那幾支
  是否集中在某個時段。
- [ ] T2: 觀察 8/10 ～ 8/12 是否延續低迷。若連三天低於 500，就不是單日雜訊，
  而是帳號級的節流或時段問題，需優先於任何內容調整處理。
- [ ] T3: 判讀 8/17 期中檢核時，**莫高窟（61.3%）與昌德宮（36.4%）都要標為
  離群值**。這兩支一謎一閉、分別是兩組的極端值，若不標註會讓
  「謎 vs 閉」的中位數比較被它們主導（沿用 7/21 立下的方法規則）。

## F34: metrics 資料正確性兩處缺陷

- 狀態: 待處理（2026-08-15 新增）
- 來源: 2026-08-15 開工時實測發現

兩個都不影響資料收集本身，但會讓週報／月報的判讀出錯。

**(1) `ig_posts.posted_date` 是 UTC 日期。** 台北早上 08:00 發的貼文，
UTC 是前一天 23:56，於是被記成前一天。實例：8/14 07:56（台北）發布的
普蘭巴南 carousel，`social_posts.published_at` 為 `2026-08-13T23:56Z`，
`ig_posts.posted_date` 因此記成 `2026-08-13`。這使得「哪天發了什麼」在
CSV 上系統性早一天，做逐日對照或期末檢核時容易錯配。**與 F33 T1 直接相關**
（該 task 已註記 `posted_date` 只有日期、需另取時分）。

- [ ] T1: 決定 `posted_date` 要改記 Asia/Taipei 日期，還是加一欄
  `posted_at_local`（後者不動歷史列、比較安全）。
- [ ] T2: 既有 456 列的歷史資料要不要回溯修正，或只在文件裡註明「8/15 之前
  為 UTC 日期」。

**(2) `retention` 來源回報的列數與實際寫入不符。** 2026-08-15 執行
`metrics.report` 時印出 `retention: +10 row(s) for 2026-08-13 → 2026-08-14`，
但 `data/metrics/retention.csv` 內容與上一次 commit 完全相同，最後一列仍停在
...
## F35: Landing 流量與 CTA 事件疑似追蹤中斷 (epic: E1)

- 狀態: 待處理（2026-08-17 新增）
- 來源: marketing/audits/weekly-2026-08-17.md（P0）

**8/10–8/16 的落地頁數據不成比例地歸零：**

| 指標 | 本週 | 前週 |
|---|---|---|
| `ga4.csv` web_active_users | **1** | 20 |
| `ga4.csv` web_new_users | **1** | 18 |
| `landing_cta.csv` 列數 | **0**（最後一列停在 8/09） | 5 天有值 |
| `landing_cta.csv` click_users / clicks | — | 16 / 49 |

同期 IG 觸及只掉 65%（18,977→6,570）。**流量掉 65%、落地頁掉 95%、CTA 事件
整週一列都沒有——三者不成比例**，先當追蹤問題查，不要直接當成流量下滑去改文案。

- [ ] T1: 確認 GA4 `download_click` 事件在正式站是否仍會送出（開瀏覽器實測一次，
  或看 GA4 即時報表）。這是最快能分辨「沒人來」與「來了但沒記到」的方法。
- [ ] T2: 確認 IG bio 的 website 仍是 `lorescape.app` 且連結可正常開啟
  （F15 的決策是 bio → 落地頁 → 商店，這條斷了整條漏斗就斷）。
- [ ] T3: 確認 8/09 之後 landing 有沒有部署過、GA4 measurement ID 或 consent
  設定有沒有被動到。
- [ ] T4: 若三項都正常、確認是真流量下滑，才把問題轉回 F15（reach→profile
  那一段），並在 F15 T4 記一筆。

**相關**：`app_funnel.csv` 的 App 端埋點**正常**（本週 8 次 first_open），
零的是 narration_start / complete——那是產品漏斗問題不是追蹤問題，見 F13 T4，
不與本項混為一談。

## F35: App 新用戶暴增但來源不明，且不是 IG 來的 (epic: E1)

- 狀態: 待查（2026-08-24 新增）
- 來源: marketing/audits/weekly-2026-08-24.md（P0）

**本週 App 端是全報告最強的訊號，而流量端同時在跌：**

| 指標 | 8/17–8/23 | 8/10–8/16 | 變化 |
|---|---|---|---|
| App first_open | **32** | 8 | +300% |
| Android 新用戶 | **27** | 6 | +350% |
| iOS 下載 | 8 | 2 | +300% |
| App Store 商品頁瀏覽 | 15 | 9 | +67% |
| App Store 曝光 | 236 | 428 | **−45%** |
| IG 帳號觸及 | 5,446 | 6,570 | **−17%** |

**曝光少了一半，下載卻多了三倍。** 這不是流量變多，是轉換率變好——或者
有一個現有欄位看不到的管道在導流。IG 觸及同期還在跌，所以這批人**不是
IG 來的**；Android 佔了 27/32，也不像 iOS 商店搜尋。

這件事的重要性在於：**如果它是一個可重複的管道，它比 IG 現在 0.73% 的
profile_views/reach 值錢得多**（見 F15 與 calendar 判準 3——IG 這條路已經
連五週卡在帳號層漏斗上）。反過來若查出是測試機或內部流量，那本週所有
App 端指標都要打折，連帶影響 F13 T5 對「42 人 cohort、D1 留存 0」的判讀
（見 F26：GA4 未濾除開發流量）。

- [ ] T1: 查 Play Console 的 acquisition 報表——`store_android.csv` 已有
  `store_listing_visitors` / `store_listing_acquisitions` 兩欄，但本週抓取
  +0 列（Play 匯出落後，最後一列停在 8/17）。等匯出跟上後先看這兩欄，
  能直接分辨「商店頁來的」還是「直接安裝的」。
- [ ] T2: 查 GA4 的 session_source / first_open 的 source-medium 維度，
  確認是自然搜尋、外部連結，還是 (not set)。
- [ ] T3: 排除內部流量嫌疑——比對 F26 的開發流量濾除狀態，以及這 27 個
  Android 新用戶的地理分布是否集中在單一裝置/地區。
- [ ] T4: 若確認是真實外部管道，回頭補一條「這個管道叫什麼、怎麼再來一次」
  進 MARKETING.md，並評估是否值得把資源從 IG 移過去。
