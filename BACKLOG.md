...
- [ ] T4（2026-08-17 新增，來源 weekly-2026-08-17）: **本週 8 個人開了 App，
  ...
  **先排除埋點嫌疑**：前週同一組欄位收得到事件，且 8/11 的 build
  ...
  是使用者真的沒按下播放。
  - [ ] T4a: 用全新安裝實機走一次首次開啟流程（iOS），記錄每一步停頓點。
    重點懷疑：定位權限拒絕後的空狀態、找不到播放入口、首屏沒給明確下一步。
    F23 的 pre-permission 說明卡若已上生產，一併確認它有沒有反效果。
  - [ ] T4b: 看 GA4 `screen_view` 序列，確認這 8 個 first_open 各自停在哪個
    畫面（需 F13 T1c 的具名 screen_view 已生效才能判讀）。
  - ⚠️ 樣本僅 8 人，單週可能是雜訊；但 narration_start 從 6 掉到 0 而
    first_open 只掉 56%，斜率不一致，值得先看一次而不是等下週。
