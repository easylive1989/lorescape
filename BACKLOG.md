...
- [ ] **App**（`frontend/`，下一輪）：待下次 build 送審才生效的累積改動——
  ...
  ⚠️ **2026-08-17 順檢：仍未送審，距 07-24 上架已 24 天**（連續第三週提出）。
  本週的具體代價：`app_funnel.csv` 的 first_open / narration_started /
  narration_completed 連兩週皆為 0，但同期 App Store 有 15 次下載
  （本週 2、前週 13）——**這 15 個新用戶的 App 內行為完全量不到**。
  週報因此無法回答「下載後有沒有真的聽故事」，而那正是 E1 漏斗的下半段。
  這個缺口每過一週就多累積一批量不到的用戶，且無法回溯補齊。
...
**相關**：`app_funnel.csv` 的 first_open / narration_* 連兩週皆 0，但同期
App Store 確有下載（本週 2、前週 13）——那是另一個缺口，成因已知（埋點修復
還沒送審上架，見「待部署」段），不與本項混為一談。
