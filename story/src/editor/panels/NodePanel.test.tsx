...
test('第一段的上移鈕與最後一段的下移鈕停用', () => {
  render(<NodePanel script={demoScript} node={node} onChange={() => {}} />)
...
test('next 型態：下拉可改指向節點', () => {
  const onChange = vi.fn()
  render(<NodePanel script={demoScript} node={nextNode} onChange={onChange} />)
...
test('choices 型態：已達 3 項時「新增選項」停用', () => {
  ...
  render(<NodePanel script={demoScript} node={threeChoices} onChange={() => {}} />)
...
test('ending 型態：編輯標題', () => {
  const onChange = vi.fn()
  render(<NodePanel script={demoScript} node={endingNode} onChange={onChange} />)
...
test('無角色可加時「新增角色」停用', () => {
  ...
  render(<NodePanel script={emptyCastScript} node={bareNode} onChange={() => {}} />)
  expect(screen.getByRole('button', { name: '新增角色' })).toBeDisabled()
})

test('背景欄位唯讀顯示檔名並停用更換鈕', () => {
  render(<NodePanel script={demoScript} node={node} onChange={() => {}} />)
