...
import type { CatalogEntry, Script } from '../engine/schema'
...
export function useStory(slug: string, deps: UseStoryDeps = {}) {
  ...
  // mount（或換 slug）時並行載入內容：script／art／index.json（取 catalogEntry 用）
  useEffect(() => {
    let cancelled = false
    setScript(null)
    setArt(null)
  ...
  useEffect(() => {
    ...
    return unsubscribe
  }, [slug, subscribe, scriptPath, artPath])
  ...
  return {
    script, art, catalogEntry, error, externalUpdate, externalUpdateSeq,
