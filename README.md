...
│   │       ├── daily_story/  # 每日一景故事
│   │       ├── journey/      # 文化足跡日誌（暫時隱藏，見下方註）
│   │       ├── trip/         # 旅程分組（暫時隱藏）
│   │       ├── export/       # 旅程故事匯出 PDF（暫時隱藏）
...
> **註：書架（文化足跡日誌）暫時隱藏。** 自 2026-08-11 起，`journey/`、`trip/`、
> `export/` 三個模組在 App 內不可達——首頁不再顯示書架入口，`/journey` 與
> `/trip*` 路由也不註冊，由 `frontend/lib/app/config/feature_flags.dart` 的
> `kBookshelfEnabled = false` 控制。程式碼、測試與既有資料都保留（故事播放完
> 仍會照常寫入 journey），旗標翻回 `true` 即整組復活。恢復前，對外文案不要再
> 提這項功能。設計說明見
